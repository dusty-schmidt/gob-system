...
mcp_server: FastMCP = FastMCP(
    name="GOB integrated MCP Server",
    instructions="""
    Connect to remote GOB instance.
    GOB is a general AI assistant controlling it's linux environment.
    GOB can install software, manage files, execute commands, code, use internet, etc.
    GOB's environment is isolated unless configured otherwise.
    """,
)
...
class ToolResponse(BaseModel):
    ...
    response: str = Field(
        description="The response from the remote GOB Instance"
    )
...
class ToolError(BaseModel):
    ...
    error: str = Field(
        description="The error message from the remote GOB Instance"
    )
    chat_id: str = Field(description="The id of the chat this message belongs to.")


SEND_MESSAGE_DESCRIPTION = """
Send a message to the remote GOB Instance.
This tool is used to send a message to the remote GOB Instance connected remotely via MCP.
"""
...
async def send_message(
    message: Annotated[
        str,
        Field(
            description="The message to send to the remote GOB Instance",
            title="message",
        ),
    ],
    attachments: (
        Annotated[
            list[str],
            Field(
                description="Optional: A list of attachments (file paths or web urls) to send to the remote GOB Instance with the message. Default: Empty list",
                title="attachments",
            ),
        ]
        | None
    ...
    Field(
        description="The response from the remote GOB Instance", title="response"
    ),
]:
...
Finish a chat with the remote GOB Instance.
This tool is used to finish a persistent chat (send_message with persistent_chat=True) with the remote GOB Instance connected remotely via MCP.
If you want to continue the chat, use the send_message tool instead.
Always use this tool to finish persistent chat conversations with remote GOB.
"""
