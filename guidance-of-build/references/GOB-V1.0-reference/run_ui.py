...
# Initialize logger first to catch any startup errors
logger = get_logger()

def run():
    try:
        logger.info("Initializing GOB framework...")
        ...
        # init_a0()  # Moved to background thread to prevent startup blocking

        # run the server
        server.serve_forever()
    except Exception as e:
        logger.exception("GOB backend failed to start due to a critical error.")
        sys.exit(1)
